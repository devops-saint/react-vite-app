...
resource "aws_dynamodb_table" "requests" {
  ...
  # lambda-personal/handler.py) - lets that lookup be a Query, not a Scan.
  global_secondary_index {
...
resource "aws_lambda_function" "request_api" {
  ...
  environment {
    variables = {
      ...
      # needs to change.
      DOMAIN = var.domain
      # Used only to build a human-viewable GitHub PR link per stage in
      # GET /dpc/requests/{id} (stage_summary) - this Lambda never calls
      # the GitHub API itself, that's still exclusively the gitops
      # Lambda's job.
